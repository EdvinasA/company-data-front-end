...
export class DeliveryInformationFormComponent implements OnInit {
  ...
  @Output() public createdDeliveryInformation =
  ...
  submitForm() {
    let info = this.converterService.convertToDeliveryInformation(
      this.shippingDeliveryForm.value
    );
  ...
  findIndexToUpdate(information: DeliveryInformation) {
    return this.user?.deliveryInformation.findIndex(
