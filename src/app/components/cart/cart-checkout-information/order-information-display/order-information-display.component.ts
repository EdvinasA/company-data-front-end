...
export class OrderInformationDisplayComponent implements OnInit {
  ...
  checkDiscountCode(input: string) {
    if (!this.isDiscountApplied) {
      ...
      this.discountService
        .checkIfDiscountIsValid(input)
        .subscribe((response) => {
          this.updateTotalSum(response.percent);
        });
    }
    if (this.isDiscountApplied) {
      this.notificationService.informationalMessage(
        'Discount for this order is already applied.',
        ''
      );
    }
  }
  ...
  updateTotalSum(percent: number) {
    if (percent != null) {
      this.totalSumOfAllItemsSubject =
        (this.totalSumOfAllItemsSubject * (100 - percent)) / 100;
    ...
    this.notificationService.informationalMessage(
      'Not existing or expired discount code was used.',
      ''
    );
  }
}
